...
class SpeedometerMeasurementController {
    ...
    async getSpeedometerCurrentMeasurement(trafficCounterId) {
        ...
        return new TrafficMeasurement(rows[0].id, rows[0].traffic_counter_id, rows[0].vehicles_count, rows[0].average_speed, rows[0].over_speed_limit_count, rows[0].created_at);
    }
}
